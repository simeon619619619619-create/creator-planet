...
const ResponseDetail: React.FC<ResponseDetailProps> = ({ response, questions, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-[#0A0A0A] rounded-xl max-w-2xl w-full max-h-[85vh] flex flex-col">
        {/* Header */}
        <div className="px-6 py-4 border-b border-[#1F1F1F] flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            />
            <div>
              <h2 className="font-semibold text-[#FAFAFA]">{response.student.full_name}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="space-y-6">
            {questions.map((question, index) => (
              <div key={question.id} className="border-b border-[#1F1F1F] pb-4 last:border-0">
                <div className="flex gap-3">
                  <span className="w-6 h-6 bg-[#1F1F1F] text-[#FAFAFA] rounded text-sm font-medium flex items-center justify-center shrink-0">
                    {index + 1}
                  </span>
                  <div className="flex-1">
                    <p className="text-sm font-medium text-[#A0A0A0] mb-2">{question.question_text}</p>
                    <p className="text-[#FAFAFA]">{getAnswerDisplay(question.id)}</p>
                  </div>
                </div>
              </div>
            ))}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0A0A0A]">
        <div className="text-center">
          ...
          <p className="mt-4 text-[#A0A0A0]">{error}</p>
          <button
            onClick={onClose}
            className="mt-4 px-4 py-2 bg-white text-black rounded-lg hover:bg-[#E0E0E0]"
          >
            {t('common.goBack')}
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#1F1F1F]">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        {responses.length === 0 ? (
          <div className="text-center py-16 bg-[#0A0A0A] rounded-xl border border-[#1F1F1F]">
            <FileSpreadsheet className="w-16 h-16 text-[#A0A0A0] mx-auto" />
            <h3 className="mt-4 text-lg font-medium text-[#FAFAFA]">
              {t('surveys.responses.noResponses')}
            </h3>
            <p className="mt-2 text-[#666666]">{t('surveys.responses.noResponsesDesc')}</p>
          </div>
        ) : (
          <>
            {/* Search */}
            ...
            <div className="bg-[#0A0A0A] rounded-xl border border-[#1F1F1F] overflow-hidden">
              <table className="w-full">
                <thead className="bg-[#0A0A0A] border-b border-[#1F1F1F]">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-[#666666] uppercase tracking-wide">
