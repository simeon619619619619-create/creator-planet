...
const PendingSurveysWidget: React.FC<PendingSurveysWidgetProps> = ({ studentId }) => {
  ...
  if (isLoading) {
    return (
      <div className="bg-[var(--fc-section,#0A0A0A)] rounded-xl p-6 border border-[var(--fc-section-border,#1F1F1F)]">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-3 bg-[#EAB308]/10 rounded-lg">
            <ClipboardCheck className="w-6 h-6 text-[#EAB308]" />
          </div>
          <div>
            <h3 className="font-semibold text-[var(--fc-section-text,#FAFAFA)]">{t('surveys.pending.title')}</h3>
          </div>
        </div>
          ...
          {pendingSurveys.map((survey) => (
            ...
            >
              <div className="flex-1 min-w-0">
                <h4 className="font-medium text-[var(--fc-section-text,#FAFAFA)] truncate">{survey.survey_title}</h4>
                {survey.community_name && (
                  <p className="text-sm text-[var(--fc-section-muted,#666666)] flex items-center gap-1 mt-0.5">
