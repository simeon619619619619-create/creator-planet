    ...
    <div className="min-h-screen bg-[var(--fc-section,#0A0A0A)]">
      ...
      <div className="bg-[var(--fc-section,#0A0A0A)] border-b border-[var(--fc-section-border,#1F1F1F)]">
        <div className="max-w-5xl mx-auto px-6 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[var(--fc-section-text,#FAFAFA)]">{t('homework.page.title')}</h1>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-[var(--fc-section,#0A0A0A)] rounded-xl p-5 border border-[var(--fc-section-border,#1F1F1F)]">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-[var(--fc-section-muted,#A0A0A0)]">{t('homework.stats.pending')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[var(--fc-section,#0A0A0A)] rounded-xl p-5 border border-[var(--fc-section-border,#1F1F1F)]">
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-[var(--fc-section-hover,#1F1F1F)] rounded-lg">
                <FileText className="w-5 h-5 text-[var(--fc-section-text,#FAFAFA)]" />
              </div>
              <div>
                <p className="text-2xl font-bold text-[var(--fc-section-text,#FAFAFA)]">
                  {submissions.length}
                </p>
                <p className="text-sm text-[var(--fc-section-muted,#A0A0A0)]">{t('homework.stats.submitted')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[var(--fc-section,#0A0A0A)] rounded-xl p-5 border border-[var(--fc-section-border,#1F1F1F)]">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-[var(--fc-section-muted,#A0A0A0)]">{t('homework.stats.pointsEarned')}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {pendingSurveys.length > 0 && (
          <section className="mb-10">
            <h2 className="text-lg font-semibold text-[var(--fc-section-text,#FAFAFA)] mb-4 flex items-center gap-2">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className="font-semibold text-[var(--fc-section-text,#FAFAFA)] text-lg">
                          {survey.survey_title}
                        </h3>
                      ...
                      {survey.survey_description && (
                        <p className="text-[var(--fc-section-muted,#A0A0A0)] mt-1 line-clamp-2">
                          {survey.survey_description}
                        </p>
                      )}
                      {survey.community_name && (
                        <p className="text-sm text-[var(--fc-section-muted,#666666)] mt-2 flex items-center gap-1">
        ...
        <section className="mb-10">
          ...
          {pendingAssignments.length === 0 ? (
            <div className="bg-[var(--fc-section,#0A0A0A)] rounded-xl border border-[var(--fc-section-border,#1F1F1F)] p-8 text-center">
            ...
            <div className="space-y-4">
              {pendingAssignments.map((assignment) => (
                <div
                  key={assignment.id}
                  className="bg-[var(--fc-section,#0A0A0A)] rounded-xl border border-[var(--fc-section-border,#1F1F1F)] p-5 hover:border-[var(--fc-section-border,#333333)] transition-all"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-[var(--fc-section-text,#FAFAFA)] text-lg">
                      ...
                      <div className="flex items-center gap-4 mt-3 text-sm">
                        <span className="flex items-center gap-1 text-[var(--fc-section-text,#FAFAFA)] font-medium">
                          ...
                          <span
                            className={`flex items-center gap-1 ${
                              isPastDue(assignment.due_date)
                                ? 'text-[#EF4444]'
                                : 'text-[var(--fc-section-muted,#666666)]'
                            }`}
                          >
        ...
        <section>
          <h2 className="text-lg font-semibold text-[var(--fc-section-text,#FAFAFA)] mb-4 flex items-center gap-2">
            <FileText className="w-5 h-5 text-[var(--fc-section-muted,#A0A0A0)]" />
            {t('homework.mySubmissions.sectionTitle')}
          </h2>

          {submissions.length === 0 ? (
            <div className="bg-[var(--fc-section,#0A0A0A)] rounded-xl border border-[var(--fc-section-border,#1F1F1F)] p-8 text-center">
              <FileText className="w-12 h-12 text-[var(--fc-section-muted,#A0A0A0)] mx-auto mb-3" />
              <h3 className="text-lg font-semibold text-[var(--fc-section-text,#FAFAFA)]">
            ...
            <div className="space-y-4">
              {submissions.map((submission) => (
                <div
                  key={submission.id}
                  className="bg-[var(--fc-section,#0A0A0A)] rounded-xl border border-[var(--fc-section-border,#1F1F1F)] p-5"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3">
                      ...
                      {submission.status === 'graded' && (
                        <div className="mt-3 p-3 bg-[var(--fc-section,#0A0A0A)] rounded-lg">
      ...
      {selectedSurvey && profile && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          ...
          <div className="relative min-h-screen flex items-center justify-center p-4">
            <div className="relative bg-[var(--fc-section,#0A0A0A)] rounded-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
              {/* Close Button */}
              <button
                onClick={() => setSelectedSurvey(null)}
                className="absolute top-4 right-4 z-10 p-2 bg-[var(--fc-section,#0A0A0A)]/90 hover:bg-[var(--fc-section,#0A0A0A)] rounded-full transition-colors"
              >
                <X className="w-5 h-5 text-[var(--fc-section-muted,#A0A0A0)]" />
              </button>
