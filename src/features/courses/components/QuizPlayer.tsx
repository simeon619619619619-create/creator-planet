  ...
  if (state === 'intro') {
    return (
      <div className="max-w-lg mx-auto text-center py-8">
        ...
        <h2 className="text-2xl font-bold text-slate-900 mb-2">{lessonTitle}</h2>
        <p className="text-slate-500 mb-6">Quiz</p>
        ...
        {error ? (
          ...
          <>
            <div className="bg-slate-50 rounded-xl p-6 mb-6">
              <div className="grid grid-cols-2 gap-4 text-left">
                <div>
                  ...
                  <p className="text-xl font-semibold text-slate-900">{questions.length}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500">Pass Score</p>
                  <p className="text-xl font-semibold text-slate-900">{PASSING_THRESHOLD}%</p>
                </div>
              </div>
              ...
              {bestAttempt && (
                <div className="mt-4 pt-4 border-t border-slate-200">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-500">Your best score</span>
                    <span
                      className={`text-lg font-semibold ${
                        bestAttempt.passed ? 'text-green-600' : 'text-amber-600'
                      }`}
                    >
            ...
            <ul className="text-left text-sm text-slate-600 mb-8 space-y-2">
              ...
              <li className="flex items-start gap-2">
                <RefreshCw size={16} className="text-indigo-600 mt-0.5 shrink-0" />
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
          ...
          {currentQuestion?.question_type === 'free_answer' ? (
            <textarea
              ...
              className="w-full px-4 py-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-slate-900 resize-none"
            />
          ) : (
        ...
        <div className="flex items-center justify-between">
          <button
            ...
            className="flex items-center gap-1 px-4 py-2 text-slate-600 hover:text-slate-900 disabled:opacity-30 disabled:cursor-not-allowed"
          >
            <ChevronLeft size={18} />
            Previous
          </button>
          ...
          {currentIndex < totalQuestions - 1 ? (
            <button
              onClick={() => goToQuestion(currentIndex + 1)}
              disabled={state === 'submitting'}
              className="flex items-center gap-1 px-4 py-2 text-indigo-600 hover:text-indigo-700 disabled:opacity-50"
            >
              Next
    ...
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <div className="space-y-3 mb-8">
          <h3 className="font-medium text-slate-900">Answer Review</h3>
