...
const UpgradeModal: React.FC<UpgradeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative bg-[#0A0A0A] rounded-xl border border-[#1F1F1F] max-w-lg w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-4 mb-6">
            {/* Current Plan */}
            <div className="flex-1 p-4 bg-[#0A0A0A] border border-[#1F1F1F] rounded-lg text-center">
              <p className="text-sm text-[#666666] mb-1">{t('billing.upgradeModal.currentPlanLabel')}</p>
              <p className="font-semibold text-[#FAFAFA]">{currentInfo.name}</p>
              <p className="text-sm text-[#A0A0A0]">{currentInfo.priceMonthly}{t('billing.upgradeModal.priceSuffix')}</p>
              <p className="text-xs text-[#666666]">{t('billing.upgradeModal.feeLabel', { fee: currentInfo.platformFee })}</p>
            </div>

            {/* Arrow */}
            <ArrowRight size={24} className="text-[#666666] shrink-0" />

            {/* New Plan */}
            <div className="flex-1 p-4 bg-[#151515] rounded-lg text-center border border-[#333333]">
              <p className="text-sm text-[#FAFAFA] mb-1">{t('billing.upgradeModal.newPlanLabel')}</p>
              <p className="font-semibold text-[#FAFAFA]">{newInfo.name}</p>
              <p className="text-sm text-[#A0A0A0]">{newInfo.priceMonthly}{t('billing.upgradeModal.priceSuffix')}</p>
              <p className="text-xs text-[#FAFAFA]">{t('billing.upgradeModal.feeLabel', { fee: newInfo.platformFee })}</p>
            </div>
          </div>

          {/* What Changes */}
          <div className="mb-6">
            <h3 className="font-semibold text-[#FAFAFA] mb-3">{t('billing.upgradeModal.whatChanges.title')}</h3>
                  ...
                  {newInfo.features
                    ...
                    .map((feature, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm">
                        <CheckCircle size={16} className="text-[#22C55E] shrink-0 mt-0.5" />
                        <span className="text-[#A0A0A0]">{t('billing.upgradeModal.whatChanges.unlockFeature', { feature })}</span>
                      </li>
                    ))}
                </>
              )}
                ...
                <>
                  ...
                  {currentInfo.features
                    ...
                    .map((feature, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm">
                        <AlertTriangle size={16} className="text-[#EAB308] shrink-0 mt-0.5" />
                        <span className="text-[#A0A0A0]">{t('billing.upgradeModal.whatChanges.loseFeature', { feature })}</span>
                      </li>
                    ))}
                </>
              )}
          ...
          {/* Billing Info */}
          <div className="p-4 bg-[#0A0A0A] border border-[#1F1F1F] rounded-lg mb-6">
          ...
          {isDowngrade && (
            <div className="p-4 bg-[#EAB308]/10 border border-[#EAB308]/20 rounded-lg mb-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium text-[#FAFAFA]">{t('billing.upgradeModal.downgradeWarning.title')}</p>
        ...
        {/* Footer */}
        <div className="flex items-center gap-3 p-6 border-t border-[#1F1F1F]">
          <button
            onClick={onCancel}
            disabled={isLoading}
            className="flex-1 py-3 px-4 border border-[#1F1F1F] text-[#FAFAFA] font-medium rounded-lg hover:bg-[#151515] hover:border-[#333333] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
