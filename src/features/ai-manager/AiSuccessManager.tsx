    ...
    <div className="max-w-6xl mx-auto p-6 h-[calc(100dvh-64px)] flex flex-col">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#FAFAFA] flex items-center gap-2">
            <Sparkles className="text-[#FAFAFA]" />
            {t('aiManager.title')}
          </h1>
          <p className="text-[#666666]">{t('aiManager.subtitle')}</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={startNewConversation}
            className="flex items-center gap-2 px-3 py-2 bg-[#1F1F1F] text-[#A0A0A0] rounded-lg hover:bg-[#1F1F1F] transition-colors text-sm font-medium"
          >
            <Plus size={16} />
          ...
          <div className="relative">
            ...
            {showConversationHistory && (
              <div className="absolute top-12 right-0 w-80 bg-[#0A0A0A] rounded-lg border border-[#1F1F1F] z-50 max-h-96 overflow-y-auto">
                <div className="p-3 border-b border-[#1F1F1F] flex justify-between items-center">
                  ...
                  <button onClick={() => setShowConversationHistory(false)} className="text-[#666666] hover:text-[#A0A0A0]">
                    <X size={16} />
                  </button>
                </div>
                <div className="p-2">
                  {conversationHistory.length === 0 ? (
                    <p className="text-sm text-[#666666] text-center py-4">{t('aiManager.history.empty')}</p>
                  ) : (
                    conversationHistory.map(conv => (
                      <button
                        key={conv.id}
                        onClick={() => loadConversationFromHistory(conv)}
                        className="w-full text-left p-3 rounded-lg hover:bg-[#0A0A0A] transition-colors"
                      >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[#1F1F1F] text-[#FAFAFA] rounded-lg hover:bg-[#1F1F1F] transition-colors disabled:opacity-50 text-sm font-medium"
          >
            {isRecalculating ? (
          ...
          <div className="bg-[#1F1F1F] p-1 rounded-lg flex">
            <button
              onClick={() => setActiveTab('chat')}
              className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'chat' ? 'bg-[#0A0A0A] text-[#FAFAFA]' : 'text-[#666666] hover:text-[#FAFAFA]'}`}
            >
              {t('aiManager.buttons.mentorChat')}
            </button>
            <button
              onClick={() => setActiveTab('report')}
              className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'report' ? 'bg-[#0A0A0A] text-[#FAFAFA]' : 'text-[#666666] hover:text-[#FAFAFA]'}`}
            >
      ...
      {activeTab === 'chat' ? (
        <div className="flex-1 flex gap-4 overflow-hidden">
          {/* Chat Section */}
          <div className="flex-1 bg-[#0A0A0A] rounded-xl border border-[#1F1F1F] overflow-hidden flex flex-col">
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'model' ? 'bg-[#1F1F1F] text-[#FAFAFA]' : 'bg-[#1F1F1F] text-[#A0A0A0]'}`}>
                    {msg.role === 'model' ? <Bot size={18} /> : <User size={18} />}
                  </div>
                  <div className={`max-w-[70%] p-4 rounded-2xl ${msg.role === 'model' ? 'bg-[#151515] border border-[#1F1F1F] text-[#A0A0A0] rounded-tl-none' : 'bg-[#FAFAFA] text-black rounded-tr-none'}`}>
                    {msg.role === 'model' ? (
              ...
              {isTyping && (
                 <div className="flex gap-4">
                   ...
                   <div className="bg-[#151515] border border-[#1F1F1F] p-4 rounded-2xl rounded-tl-none flex gap-1">
            ...
            <div className="p-4 bg-[#0A0A0A] border-t border-[#1F1F1F]">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-[#0A0A0A] border border-[#1F1F1F] rounded-lg px-4 py-2 text-[#FAFAFA] placeholder:text-[#666666] focus:border-[#555555] focus:ring-1 focus:ring-white/10 focus:outline-none"
                />
                <button
                  onClick={handleSend}
          ...
          <div className="w-80 bg-[#0A0A0A] rounded-xl border border-[#1F1F1F] overflow-hidden flex flex-col">
            <div className="p-4 border-b border-[#1F1F1F]">
              {/* Status Filter Tabs */}
              <div className="flex gap-1 p-1 bg-[#1F1F1F] rounded-lg mb-3">
                {[
                  ...
                  { value: 'top_member' as const, label: t('aiManager.filters.top'), icon: Star, activeColor: 'text-[#FAFAFA] bg-[#1F1F1F]' },
                  { value: 'all' as const, label: t('aiManager.filters.all'), icon: Users, activeColor: 'text-[#A0A0A0] bg-[#0A0A0A]' },
                  ...
                  <div
                    ...
                    className={`p-3 rounded-lg border transition-colors cursor-pointer ${
                      student.status === 'at_risk'
                        ...
                        ? 'bg-[#1F1F1F] border-[#333333] hover:border-[#555555]'
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-2">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-xs text-[#A0A0A0]">{t('aiManager.studentCard.riskScore')}</span>
        ...
        <div className="flex-1 bg-[#0A0A0A] rounded-xl border border-[#1F1F1F] p-8 flex flex-col items-center">
          {!report ? (
            <div className="text-center max-w-md">
              <div className="w-16 h-16 bg-[#1F1F1F] rounded-full flex items-center justify-center mx-auto mb-6 text-[#FAFAFA]">
                <FileText size={32} />
              </div>
              <h3 className="text-xl font-bold text-[#FAFAFA] mb-2">{t('aiManager.report.title')}</h3>
            ...
            <div className="w-full max-w-3xl animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex justify-between items-center mb-6">
                 <h2 className="text-xl font-bold text-[#FAFAFA]">{t('aiManager.report.resultsTitle')}</h2>
                 <button onClick={() => setReport(null)} className="text-sm text-[#FAFAFA] font-medium hover:underline">{t('aiManager.buttons.runNewAnalysis')}</button>
              </div>
              <div className="bg-[#0A0A0A] p-6 rounded-xl border border-[#1F1F1F]">
                <AiResponseText text={report} className="text-[#FAFAFA]" />
              </div>
            </div>
          )}
        </div>
