...
export function EngagementDashboard({ engagement }: EngagementDashboardProps) {
  ...
  return (
    <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded-xl overflow-hidden">
      {/* Header */}
      <div className="bg-[#151515] p-6 border-b border-[#1F1F1F]">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="text-xl font-bold text-[#FAFAFA]">{packageName}</h3>
            <p className="text-[#A0A0A0] mt-1">{t('dwyPackages.engagementDashboard.activeEngagement')}</p>
          </div>
          <span className={`px-3 py-1 rounded-full text-sm font-medium bg-[#0A0A0A]/20`}>
      ...
      <div className="p-6 border-b border-[#1F1F1F]">
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <div className="text-sm text-[#666666]">{t('dwyPackages.engagementDashboard.startedLabel')}</div>
            <div className="font-medium text-[#FAFAFA]">{formatDate(engagement.started_at)}</div>
          </div>
          <div>
            <div className="text-sm text-[#666666]">{t('dwyPackages.engagementDashboard.targetCompletionLabel')}</div>
            <div className="font-medium text-[#FAFAFA]">{formatDate(engagement.expected_end_at)}</div>
          </div>
          <div>
            <div className="text-sm text-[#666666]">{t('dwyPackages.engagementDashboard.daysRemainingLabel')}</div>
      ...
      {/* Quick links */}
      <div className="p-6 border-b border-[#1F1F1F]">
        <h4 className="font-medium text-[#FAFAFA] mb-3">{t('dwyPackages.engagementDashboard.quickLinksTitle')}</h4>
        <div className="flex flex-wrap gap-2">
          {engagement.scope_document_url && (
            <a
              ...
              className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#1F1F1F] rounded-lg text-sm text-[#A0A0A0] hover:bg-[#1F1F1F] transition-colors"
            >
        ...
        {!engagement.scope_document_url && !engagement.project_folder_url && !engagement.slack_channel && (
          <p className="text-[#666666] text-sm">{t('dwyPackages.engagementDashboard.linksOnboarding')}</p>
        )}
      </div>
      ...
      {engagement.notes && (
        <div className="p-6 border-t border-[#1F1F1F] bg-[#0A0A0A]">
          <h4 className="font-medium text-[#FAFAFA] mb-2">{t('dwyPackages.engagementDashboard.notesTitle')}</h4>
          <p className="text-[#A0A0A0] text-sm whitespace-pre-wrap">{engagement.notes}</p>
        </div>
      )}
    </div>
  );
