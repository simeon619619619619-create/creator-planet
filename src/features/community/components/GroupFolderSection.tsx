  ...
  return (
    <div className="mb-2">
      ...
      <button
        ...
        className="group w-full flex items-center gap-1 px-2 py-1.5 text-xs font-semibold text-[#A0A0A0] hover:text-[#FAFAFA] uppercase tracking-wider"
      >
        {isExpanded ? (
          <ChevronDown size={14} />
        ) : (
        ...
        {memberCount !== undefined && !isGlobal && (
          <span className="text-[10px] bg-[#1F1F1F] text-[#A0A0A0] px-1.5 py-0.5 rounded-full font-normal normal-case">
            {memberCount}
          </span>
        )}
        {isOwner && onAddChannel && (
          <span
            ...
            className="p-0.5 hover:bg-[#1F1F1F] rounded opacity-0 group-hover:opacity-100 transition-opacity"
          >
            <Plus size={12} />
          </span>
        )}
      ...
      {isExpanded && (
        <div className="space-y-0.5 mt-0.5">
              ...
              {isOwner && (
                <div className="relative">
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-1 text-[#666666] hover:text-[#A0A0A0] transition-all"
                  >
                    <MoreHorizontal size={14} />
                  </button>
                  {showChannelMenu === channel.id && (
                    <div className="absolute right-0 top-full mt-1 bg-[#0A0A0A] rounded-lg  border border-[#1F1F1F] py-1 z-20 min-w-[100px]">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className="w-full text-left px-3 py-1.5 text-sm text-[#FAFAFA] hover:bg-[#151515] flex items-center gap-2"
                      >
                        <Edit3 size={12} />
          ...
          {channels.length === 0 && (
            <p className="px-3 py-2 text-xs text-[#666666] italic">
